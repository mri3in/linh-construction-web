    ...
    function anps_header_media($id, $image_size) {
        ...
        if(get_post_meta($id, $key ='gallery_images', $single = true )) {
            ...
            foreach($gallery_images as $key=>$item) {
                $image_src = wp_get_attachment_image_src($item, $image_size);
...
if(!function_exists('anps_menu_is_centered')) {
    function anps_menu_is_centered() {
        ...
        if($global_menu_center_checkbox == 1) {
...
if(!function_exists('anps_get_menu')) {
    ...
    ?>
        <div class="mobile-wrap">
            ...
            /* Max mega menu */
            $menu_class_id = 'main-menu';
            if (is_plugin_active('megamenu/megamenu.php') && class_exists('Mega_Menu')) {
                $menu_class_id = '';
            }
            /* END Max mega menu */
            ...
            wp_nav_menu( array(
                'container' => false,
                'menu_class' => $menu_class_id,
                'menu_id' => $menu_class_id,
                'echo' => true,
                'before' => '',
                'after' => '',
                'link_before' => '',
...
if(!function_exists('anps_theme_options_add_page')) {
    function anps_theme_options_add_page() {
        if(current_user_can('manage_options')) {
            add_theme_page(esc_html__('Theme Options', 'construction'), esc_html__('Theme Options', 'construction'), 'read', 'theme_options', 'anps_theme_options_do_page');
        }
        ...
        if(current_user_can('manage_options') && is_plugin_active('anps_theme_plugin/anps_theme_plugin.php' ) && function_exists('anps_portfolio')) {
            add_theme_page(esc_html__('Plugin Options', 'construction'), esc_html__('Plugin Options', 'construction'), 'read', 'anps_plugin_options', 'anps_plugin_options_do_page');
        }
    }
}
    ...
    function anps_setup() {
        ...
        if(get_option('anps_home_classic_menu_type', 'top') == 'style4') {
            register_nav_menus(array(
                'primary' => esc_html__('Left Navigation', 'construction'),
                'anps_right' => esc_html__('Right Navigation', 'construction'),
            ));
        } else {
            register_nav_menus(array(

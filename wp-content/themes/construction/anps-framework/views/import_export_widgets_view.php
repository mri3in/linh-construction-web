...
if(isset($_GET['save_file_widget'])) {
    $import_export->save_file_widgets();
}
/* Import widgets data */
if (isset($_GET['import_widgets'])) {  
    $import_export->import_widgets_data();
}
?>
<div class="content-inner">
    <h3><?php esc_html_e('Export widgets', 'construction'); ?></h3>
    <form action="themes.php?page=theme_options&sub_page=import_export_widgets&save_file_widget" method="post">
        <div class="wrap">
            <button type="submit" class="inline-save btn-std"><i class="fa fa-floppy-o"></i><?php esc_html_e('Export', 'construction'); ?></button>
        </div>
    </form>

    <h3><?php esc_html_e('Import widgets', 'construction'); ?></h3>
    <form action="themes.php?page=theme_options&sub_page=import_export_widgets&import_widgets" method="post" enctype="multipart/form-data">
        <div class="wrap">
            ...
            <button type="submit" class="inline-save btn-std"><i class="fa fa-floppy-o"></i><?php esc_html_e('Import', 'construction'); ?></button>
        </div>    
    </form>
</div>